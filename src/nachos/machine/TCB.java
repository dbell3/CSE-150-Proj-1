...
public final class TCB {
	...
	public void start(Runnable target) {
		/**
		 * We will not use synchronization here, because we're assuming that either this
		 * is the first call to start(), or we're being called in the context of another
		 * TCB. Since we only allow one TCB to run at a time, no synchronization is
		 * necessary.
		 *
		 * The only way this assumption could be broken is if one of our non-Nachos
		 * threads used the TCB code.
		 */

		/**
		 * Make sure this TCB has not already been started. If done is false, then
		 * destroy() has not yet set javaThread back to null, so we can use javaThread
		 * as a reliable indicator of whether or not start() has already been invoked.
		 */
		Lib.assertTrue(javaThread == null && !done);

		/**
		 * Make sure there aren't too many running TCBs already. This limitation exists
		 * in an effort to prevent wild thread usage.
		 */
		...
		/**
		 * Probably unnecessary sanity check: if this is not the first TCB, we make sure
		 * that the current thread is bound to the current TCB. This check can only fail
		 * if non-Nachos threads invoke start().
		 */
		if (!isFirstTCB)
			Lib.assertTrue(currentTCB.javaThread == Thread.currentThread());

		/**
		 * At this point all checks are complete, so we go ahead and start the TCB.
		 * Whether or not this is the first TCB, it gets added to runningThreads, and we
		 * save the target closure.
		 */
		...
		if (!isFirstTCB) {
			/**
			 * If this is not the first TCB, we have to make a new Java thread to run it.
			 * Creating Java threads is a privileged operation.
			 */
			...
			/**
			 * The Java thread hasn't yet started, but we need to get it blocking in
			 * yield(). We do this by temporarily turning off the current TCB, starting the
			 * new Java thread, and waiting for it to wake us up from threadroot(). Once the
			 * new TCB wakes us up, it's safe to context switch to the new TCB.
			 */
			...
			/*
			 * This is the first TCB, so we don't need to make a new Java thread to run it;
			 * we just steal the current Java thread.
			 */
	...
	public void contextSwitch() {
		/*
		 * Probably unnecessary sanity check: we make sure that the current thread is
		 * bound to the current TCB. This check can only fail if non-Nachos threads
		 * invoke start().
		 */
		Lib.assertTrue(currentTCB.javaThread == Thread.currentThread());
		...
		/**
		 * There are some synchronization concerns here. As soon as we wake up the next
		 * thread, we cannot assume anything about static variables, or about any TCB's
		 * state. Therefore, before waking up the next thread, we must latch the value
		 * of currentTCB, and set its running flag to false (so that, in case we get
		 * interrupted before we call yield(), the interrupt will set the running flag
		 * and yield() won't block).
		 */
	...
	private void threadroot() {
		...
		if (!isFirstTCB) {
			/*
			 * start() is waiting for us to wake it up, signalling that it's OK to context
			 * switch to us. We leave the running flag false so that we'll still run if a
			 * context switch happens before we go to sleep. All we have to do is wake up
			 * the current TCB and then wait to get woken up by contextSwitch() or
			 * destroy().
			 */
			...
			/*
			 * start() called us directly, so we just need to initialize a couple things.
			 */
